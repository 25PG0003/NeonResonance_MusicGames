...
void Matrix::Ortho(float width, float height, float znear, float zfar)
{
	_11 = 2.0f / width;
	_22 = 2.0f / height;
	_33 = 1.0f / (znear - zfar);
...
void Matrix::RotationXYZ(float ax, float ay, float az)
{
	float	sx, sy, sz, cx, cy, cz;
	...
	_11 = (float)(cy * cz + sx * sy * sz);
	_12 = (float)(cx * sz);
	_13 = (float)(-sy * cz + sx * cy * sz);
	_14 = (float)(0.0f);

	_21 = (float)(-cy * sz + sx * sy * cz);
	_22 = (float)(cx * cz);
	_23 = (float)(sy * sz + sx * cy * cz);
	_24 = (float)(0.0f);
